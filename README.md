...
npx pnpm@10 protoshare list
npx pnpm@10 protoshare list --json
npx pnpm@10 protoshare rm checkout
npx pnpm@10 protoshare open checkout
```

Writes `.protoshare/out/<slug>/index.html` + `shots/*.png` and serves a local gallery. The share URL is copied, opened in the browser, and printed as a terminal QR (`--no-copy` / `--no-browser` / `--no-qr` skip). `--pack` also writes `gallery.tgz` (frozen share you can send without a live tunnel). `--upload-url` PUTs that archive to a presigned S3/R2 URL (`PROTOSHARE_UPLOAD_URL`); or set `PROTOSHARE_S3_ENDPOINT` + `PROTOSHARE_S3_BUCKET` + `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` for SigV4 PUT. `--public-url` / `PROTOSHARE_PUBLIC_URL` is what we print. Each share is also recorded in libsql (`PROTOSHARE_LIBSQL_URL` / `TURSO_DATABASE_URL`, default `file:.protoshare/shares.db`). `protoshare list` / `list --json` shows the catalog (`local` when `.protoshare/out/<slug>` still exists); `protoshare open <slug>` serves `.protoshare/out/<slug>` again and tries a public zrok/cloudflared URL (`--no-live` stays local); a live URL is written back to the catalog. `protoshare rm <slug>` removes a row and deletes `.protoshare/out/<slug>` (`--no-files` keeps snapshots). With zrok enabled, `--slug` also tries a vanity host (`https://<slug>.share.zrok.io`); if the name is taken, the share falls back to a random live URL.
