...
Writes `.protoshare/out/<slug>/index.html` + `shots/*.png` and serves a local gallery on `127.0.0.1:4177`. The **session** URL (live tunnel, then uploaded object, then local gallery) is copied, opened in the browser, and printed as a terminal QR (`--no-copy` / `--no-browser` / `--no-qr` skip). `--watch` ignores `--copy` / `--browser` / `--qr` — the overlay owns that UX.
